...
fn render_lap_table(
    ...
    let his_list_len = hisoty_str_list.len();

    let mut render_one_lap = |i: usize, time_str: &str| {
        let time_s = if i > 0 { time_str } else { &cur_time_str };
        let lap_str = if i > 0 {
            i.to_string()
        } else {
    ...
    if his_list_len > 0 {
        for i in (min_idx..=his_list_len).rev() {
            ...
            render_one_lap(i, &hisoty_str_list[i - 1]);
        }
    }
